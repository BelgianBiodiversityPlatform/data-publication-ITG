...
event %<>%
   mutate(dwc_decimalLongitude = (longitude)) %>%
...
```{r}
event %<>%   
   mutate(dwc_decimalLatitude = as.character(format(latitude, nsmall = 5))) %>%
   mutate(dwc_decimalLongitude = as.character(format(longitude, nsmall = 5)))
