...
```{r}
occurrence <- input_data_occurrence
...
```{r}
event <- event %<>% mutate(rowID = row_number())
```
...
```{r}
 event %<>% mutate(dwc_eventDate = paste((str_trim(Start.date)),"/",(str_trim(End.Date))))
```


```{r}
event %<>% mutate(dwc_eventDate = str_replace_all(dwc_eventDate, " ",""))
```

### samplingProtocol

```{r}
event %<>% mutate(dwc_samplingProtocol = Description)
```
...
```{r}
event %<>% mutate(dwc_locationID = str_c("ITG:FAVV:",Site.Code)) # e.g. "Belgium = BE"
```
