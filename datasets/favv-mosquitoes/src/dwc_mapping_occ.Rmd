...
```{r}
input_data %<>% mutate(rowID = row_number())
```


```{r}
input_data %<>% mutate(dwc_eventID = str_c("IGT:FAVV:EV:",str_replace(End.Date, ",","-") ,str_replace(Latitutde, ",","-"),str_replace(Longitude, ",","-"),str_replace(Start.Date,",","-"), Trap.Code))

```

# Occurrence core & eventCore

# CReate occurrence Only file, event not doable
...
```{r}
occurrence <- event %>% mutate(rowtID = row_number())
```
...
```{r}
occurrence %<>% mutate(dwc_datasetID = "complete")
```
...
```{r}
occurrence %<>% mutate(dwc_eventDate = paste((trimws(Start.Date)),"/",(End.Date))) 
...
```{r}
occurrence %<>% mutate(dwc_samplingProtocol = Trap.type)
```
...
```{r eval=FALSE, include=FALSE}
occurrence %<>% mutate(dwc_habitat = pbs_type)
```
...
```{r}
occurrence %<>% mutate(dwc_locationID = str_c("ITG:FAVV:",Collection.sites_Site.Code)) # e.g. "Belgium = BE"
```
