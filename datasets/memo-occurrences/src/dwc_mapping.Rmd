...
```{r}
integer_part_lat <- map_chr(input_data$latitude, function(x) str_split(x, pattern = "\\.")[[1]][1])

decimal_part_lat <- map(input_data$latitude, function(x) {
...
integer_part_long <- map_chr(input_data$longitude, function(x) str_split(x, pattern = "\\.")[[1]][1])

decimal_part_long <- map(input_data$longitude, function(x) {
...
input_data %>%
 mutate(decimalLatitude = as.numeric(paste(integer_part_lat, decimal_part_lat, sep = "."))) %>%
 mutate(decimalLongitude = as.numeric(paste(integer_part_long, decimal_part_long, sep = ".")))
```
