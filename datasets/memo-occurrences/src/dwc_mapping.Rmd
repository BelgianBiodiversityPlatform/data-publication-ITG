...
```{r}
event %<>% select(-c(latitude,longitude, trap_status,year, poe_latitude, poe_longitude, species.f, sex_collected, sex_stored))
```
## eventID
...
```{r}
event %<>% rename(decimalLongitude = X) %>%
...
```{r}
event %<>% mutate(locationID = SubAreaName)
```
